...
def Share_essay():
    print("\n【【【【【【【分享】】】】】】】\n")
    groupId = traversal_xiaoquan()#小圈板块
    ...
    data = {
        'articleId': random.choice(groupId),
        'forwardTo': '1'
    }
...
def insertArtComment():
    ...
    index = random.randint(0, len(openId_id_list)-1)#把随机序列号存入变量
    openId = openId_id_list[index]
    groupId = group_id_list[index]
    content = traversal_comment(openId, groupId)
    print(f"获取随机评论内容：{content}")
    data = {
        "content": content,
        "parentId": None,
        "openId": openId,
...
if __name__ == '__main__':
    ...
    for Authorization in quantity:
        ...
        if func is not None:
            ...
            insertArtComment()
            # print("【随机发布动态或文章】")
            # addArticle_list = [addArticle_1, addArticle_2]
            # addArticle = random.choice(addArticle_list)
            # msg += addArticle()
            # print("【删除】")
            # del_article()
            information()
...
#    print(msg)
    ...
    u_token = get_envs("NZtoken")#查询环境变量信息
    put_envs(u_token[0].get('id'), u_token[0].get('name'), token_list, '哪吒汽车 ' + nowtime)
    msg += f"变量更新时间：{nowtime}"
    send('哪吒汽车', msg)
